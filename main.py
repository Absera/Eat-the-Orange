...
icon = pygame.image.load("assets/orange.png")
...
class Orange(GameObject):
    ...
    '''
    image = pygame.image.load("assets/orange.png")
...
class Enemy(GameObject):
    ...
    '''
    amount = 10
    image = [pygame.image.load("assets/enemy.png") for i in range(amount)]
...
class Score:
    ...
    fontSize = 30
    highScore = None
    score_path = "C:\\Users\\Public\\eto_scores.txt"

    def increase(self):
        self.currentScore += 1

    def show(self):
        high_score = self.getHighScore()
        high_score_string = f"\{high_score}" if high_score > int(self.currentScore) else " "
        pygame.font.init()
        myfont = pygame.font.SysFont('Century', self.fontSize)
        textsurface = myfont.render("Score: {0}{1}".format(self.currentScore, high_score_string), False, (0, 0, 0))
        window.blit(textsurface, (self.xPos, self.yPos))
        self.updateHighScore()

    def updateHighScore(self):
        if not os.path.exists(self.score_path):
            temp_score = open(self.score_path, 'w')
            temp_score.write(str(0))
            temp_score.close()
        with open(self.score_path, "r+") as file:
            score = file.read()
            if int(score) < int(self.currentScore):
                with open(self.score_path, "w+") as file:
                    file.write(str(self.currentScore))
...
class Game:
    ...
    def showPlayAgain(self):
        pygame.font.init()
        high_score = score.getHighScore()
        myfont = pygame.font.SysFont('Century', 30)
        textsurface = myfont.render(f"You scored {score.currentScore}", False, color.secondary)

        textWidth = textsurface.get_width()
        textHeight = textsurface.get_height()
        window.blit(textsurface, (halfWindowWidth - textWidth / 2, halfWindowHeight - textHeight / 2 - 140))

        textHighScore = myfont.render(f"High Score {high_score}", False, color.secondary)
        textHighScoreWidth = textHighScore.get_width()
        textHighScoreHeight = textHighScore.get_height()
        window.blit(textHighScore,
                    (halfWindowWidth - textHighScoreWidth / 2, halfWindowHeight - textHighScoreHeight / 2 - 100))
...
class Button:
    ...
    def draw(self):
        ...
        window.blit(text, (
            self.xPos + (self.width / 2 - text.get_width() / 2), self.yPos + (self.height / 2 - text.get_height() / 2)))
...
class Sound:
    ...
    orange_sound = pygame.mixer.Sound("assets/orangeEaten.ogg")
    collision_sound = pygame.mixer.Sound("assets/collide.ogg")
    game_over_sound = pygame.mixer.Sound("assets/gameOver.ogg")

    def playOrangeEaten(self):
        pygame.mixer.Sound.play(self.orange_sound)
...
buttonPlayAgain = Button(color.secondary, color.primary, halfWindowWidth - 100, halfWindowHeight - 50, 200, 50,
                         "Play Again")
